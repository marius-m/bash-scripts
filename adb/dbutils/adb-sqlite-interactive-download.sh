...
while true; do
    if [ -f ${TMP_FILE_SQLITE} ]; then
        rm ${TMP_FILE_SQLITE}
    fi


    TMP_FILE_SQLITE=$(uuidgen)
    touch $TMP_FILE_SQLITE
    ...
    echo "- Pulling database file to local storage (${INNER_FILE_DB} -> ${TMP_FILE_SQLITE})"
    ...
    sqlite3 -line $TMP_FILE_SQLITE "${query}"
done
